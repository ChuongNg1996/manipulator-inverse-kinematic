...
static const int joint_num = 7;                                         // NUmber of joints
static const float home_joint[joint_num] = {0.001,1.001,0.001,0.001,0.001,0.001,0.001};       // Home position in joint (rad)
...
baxter_core_msgs::JointCommand joint_pose_cmd;
...
void JointStateCallback (const sensor_msgs::JointState& msg)
{
    ...
    */
    if (ik_command == 1)
    {
        ...
        {
            current_joint[i] = msg.position[i+3];
        }
   ...
   if (command_en == 1)
   {
        for (int i = 0; i< joint_num; i++)
        {
            error[i] = abs(joint_pose_cmd.command[i] - msg.position[i+3]); 
        }
        // error[0] = abs(joint_pose_cmd.command[0] - msg.position[2]); 
        // error[1] = abs(joint_pose_cmd.command[1] - msg.position[1]); 
        // error[2] = abs(joint_pose_cmd.command[2] - msg.position[0]); 
        // error[3] = abs(joint_pose_cmd.command[3] - msg.position[3]); 
        // error[4] = abs(joint_pose_cmd.command[4] - msg.position[4]); 
        // error[5] = abs(joint_pose_cmd.command[5] - msg.position[5]); 
...
int main(int argc, char **argv)
{
    ...
    //-------------         Topics              -------------//
    ros::Subscriber joint_state =  n.subscribe("/robot/joint_states", 1, JointStateCallback);
    ros::Subscriber goal_command =  n.subscribe("/pose_command", 1, PoseCommandCallback);
    ros::Publisher joint_command = n.advertise<baxter_core_msgs::JointCommand>("/robot/limb/left/joint_command", 1);

    //-------------         Register names for Joint Command              -------------//
    joint_pose_cmd.names.push_back("left_e0");
    joint_pose_cmd.names.push_back("left_e1");
    joint_pose_cmd.names.push_back("left_s0");
    joint_pose_cmd.names.push_back("left_s1");
    joint_pose_cmd.names.push_back("left_w0");
    joint_pose_cmd.names.push_back("left_w1");
    joint_pose_cmd.names.push_back("left_w2");

    //-------------         Joint Command: Home Position        -------------//
    std::copy (home_joint,home_joint + joint_num,joint_cmd);
    for (int i = 0; i < joint_num; i++)
    {
        joint_pose_cmd.command.push_back(joint_cmd[i]);
    }
    command_en = 1;
    while (joint_achieved == 0) joint_command.publish(joint_pose_cmd);
    joint_pose_cmd.command.clear();
    command_en = 0;
    joint_achieved = 0;
